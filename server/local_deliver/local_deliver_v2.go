...
// 赋值端口和参数
func GenInfoDeliver(submitserver_port string, orderserver_port string, userconfig global.ConfigUser) *InfoDeliver {
	ifd := &InfoDeliver{}
...
func (I *InfoDeliver) Start() {
	...
	go orderserver.OrderServerListen(I.orderserver_port, I.userconfig)
...
func (I *InfoDeliver) DealRequest(new_request *deliver.LocalSubmit) {
	...
	if requestMap["order"] || requestMap["account"] || requestMap["position"] {
		...
		go func() {
			deliver_server.DeliverAccount(I.userconfig, account_port, account_sub, order_sub, position_sub)
			l.Lock()
			judge_count += 1
			l.Unlock()
		}()
...
func main() {
	...
	ifd := GenInfoDeliver("6101", "6102", config.UserInfo["1"])
	ifd.Start()
}
