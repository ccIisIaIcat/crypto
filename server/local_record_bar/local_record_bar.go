...
func GatherSwap() {
	...
	// 创建对应的stmt
	for i := 0; i < len(swap_insid); i++ {
		sql := "insert into " + "`" + swap_insid[i] + "`" + " (Insid,Ts_open,Open_price,High_price,Low_price,Close_price,Vol,VolCcy,VolCcyQuote,FundingRate,NextFundingRate,Ts_FundingRate,TS_NextFundingRate,Oi,OiCcy,Ts_oi) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
	...
	for {
		...
		_, err := ms.Stmt_map[temp.Insid].Exec(temp.Insid, temp.Ts_open, temp.Open_price, temp.High_price, temp.Low_price, temp.Close_price, temp.Vol, temp.VolCcy, temp.VolCcyQuote, temp.FundingRate, temp.NextFundingRate, temp.Ts_FundingRate, temp.TS_NextFundingRate, temp.Oi, temp.OiCcy, temp.Ts_oi)
		if err != nil {
			log.Println(err)
		}
	}
...
func GatherSpot() {
	...
	// 创建对应的stmt
	for i := 0; i < len(spot_insid); i++ {
		sql := "insert into " + "`" + spot_insid[i] + "`" + " (Insid,Ts_open,Open_price,High_price,Low_price,Close_price,Vol,VolCcy,VolCcyQuote,Oi,OiCcy,Ts_oi) values(?,?,?,?,?,?,?,?,?,?,?,?);"
	...
	for {
		temp := <-query_obj.Bar_info_chan
		ms.Stmt_map[temp.Insid].Exec(temp.Insid, temp.Ts_open, temp.Open_price, temp.High_price, temp.Low_price, temp.Close_price, temp.Vol, temp.VolCcy, temp.VolCcyQuote, temp.Oi, temp.OiCcy, temp.Ts_oi)
	}
}
...
func GatherFutureQuarter() {
	...
	// 创建对应的stmt
	for i := 0; i < len(future_insid); i++ {
		sql := "insert into " + "`" + future_insid[i] + "`" + " (Insid,Ts_open,Open_price,High_price,Low_price,Close_price,Vol,VolCcy,VolCcyQuote,Oi,OiCcy,Ts_oi) values(?,?,?,?,?,?,?,?,?,?,?,?);"
	...
	for {
		temp := <-query_obj.Bar_info_chan
		_, err := ms.Stmt_map[temp.Insid].Exec(temp.Insid, temp.Ts_open, temp.Open_price, temp.High_price, temp.Low_price, temp.Close_price, temp.Vol, temp.VolCcy, temp.VolCcyQuote, temp.Oi, temp.OiCcy, temp.Ts_oi)
		if err != nil {
			log.Println(err)
		}
	}
