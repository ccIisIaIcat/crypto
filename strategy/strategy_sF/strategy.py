...
class strategySc(strategyBasic.strategy):
    ...
    def UpdateTick(self,tick_info):
        ...
        time_local = time.localtime(self.tick_list.GetTsByTail(1)[0]/1000)
        # if time_local.tm_min == 17:
        self.temp_tick.append(self.tick_list.GetAsk1PriceByTail(1)[0])
        if len(self.temp_tick)>3 and self.temp_tick[-1] - self.temp_tick[len(self.temp_tick)-3] > 0:
            print("a",self.test_a)
        if len(self.temp_tick)>3 and self.temp_tick[-1] - self.temp_tick[len(self.temp_tick)-3] < 0 :
            print("b",self.test_a)
